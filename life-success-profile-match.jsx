...
function ContributionBreakdown({ row }) {
  return <dl className="contribution-breakdown" aria-label="Model component breakdown">
    <div><dt>Elevation component</dt><dd>{formatScore(row.elevation,'combined')}</dd></div>
    <div><dt>Pattern component</dt><dd>{formatScore(row.pattern,'combined')}</dd></div>
    <div><dt>Combined index</dt><dd>{formatScore(row.combined,'combined')}</dd></div>
  </dl>;
}
...
function ClusterDetails({ row, z, mode }) {
  ...
  return <div className="cluster-details">
    ...
    <p>This cluster contains {row.rows.length} research variables in total, including its exemplar. The researchers grouped variables by similarities in their Big Five regression-weight patterns, not simply by shared subject matter. The cluster name is an interpretive label, not a single measured success scale.</p>
...
function MatchResults({ profile, delta, setDelta, mode }) {
  ...
  return <>
    <section className="results-section" aria-labelledby="families-heading">
      ...
      <p className="section-intro">{isShape ? 'A higher score means a closer relative trait shape. It does not estimate your chance of success, ability or personal values.' : mode === 'elevation' ? 'This view isolates the component associated with your overall trait level in each research index. It excludes your relative trait pattern. A higher component value is not a higher probability of success.' : 'This index combines elevation and pattern using each research variable’s own regression model, then averages those outputs. Higher means a larger model index, not a greater proven chance of success.'}</p>
      {!isShape && <p className="model-note">Approximate research model · Equal weight per variable · 0 is the model’s reference mean. These are mean criterion SD units, not standard deviations of a validated family or type scale. {profile.status !== 'ready' && 'Your shape has little or no differentiation; the model remains calculable from elevation. An exactly flat profile has a zero pattern component.'}</p>}
    ...
    <section className="sensitivity-panel" aria-labelledby="sensitivity-heading">
      ...
      <p>“Tested positions” shows how each result’s position changes when each input is decreased by {delta}, unchanged, or increased by {delta} percentile point{delta > 1 ? 's' : ''}, in every combination. All five traits change together in each scenario, so the resulting changes in every match are evaluated together.</p>
...
function Method() {
  return <section className="method-page" aria-labelledby="method-heading">
    ...
    <h3>Variance accounted for by the research models</h3>
    <table><caption>Study summaries across criteria, from Tables 7–8</caption><thead><tr><th scope="col">Criterion group</th><th scope="col">Mean model R²</th></tr></thead><tbody>
    ...
    <p>These are averages of the reported R² values, not the square of the average correlation. The values summarize variance statistically accounted for by the models across the broader review. They do not establish that personality caused an outcome, measure this app’s accuracy, or estimate your chance of success.</p>
    <p>Of the personality-related variance in the paper’s models, roughly 71% is associated with overall profile level and 29% with profile pattern. These are research summaries, not weights for mixing the two scores. This app offers pure shape similarity and an approximate linear model with separate elevation and pattern components.</p>
    ...
    <p>The authors propose individual profile matching as an application. This specific scoring and display implementation has not been independently validated. The evidence is mostly cross-sectional and correlational. Summary statistics support the regression reconstruction, but they do not establish out-of-sample or individual-level predictive validity for these indices.</p>

    <h3>Elevation and the combined model</h3>
    <p>Your descriptive elevation is the average of the five normal-score coordinates. It is not the average of your input percentiles and is not itself a population-standardized composite. The Elevation component view shows the model component associated with that common level for each research index. The Combined model adds the criterion-specific pattern component.</p>
    ...
    <p>The pattern component is a weighted linear sum, not the Pearson shape similarity. It retains the magnitude of your trait differences. Elevation and pattern add exactly before rounding. They are not assumed to be statistically independent; no 71/29 weighting or arbitrary blend is used.</p>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      <div className="hero"><span className="eyebrow">Big Five × forms of success</span><h1>Explore your personality across the forms of success.</h1><p>Compare your overall trait level and relative pattern across ten research-derived types and three broader families. Explore the two model components together or separately.</p></div>
